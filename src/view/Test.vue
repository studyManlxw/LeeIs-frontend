<template>
    <div class="box">
        <van-divider dashed style="color: #F9F5E7">生成订单</van-divider>
        <van-cell title="注意：点击店员类型开始按顺序依次选择" size="large"/>

        <!--        选择性别-->
        <van-cell title="店员信息" size="large"
                  :label="'所需类型：' + ` ${sex == ''? '待选择': radio == 0 ? '女':'男' }`"
                  @click="showSex =true"/>
        <template>
            <van-radio-group style="background: #F9F5E7" direction="horizontal" icon-size="22px" v-model="radio" @change="radioSex(radio)"
                             v-show="showSex">
    ...
    methods: {
        ...
        onSubmit() {
            if (this.finalData.wxNumber.length != 0) {
                request.post("/order/submit/", this.finalData).then((res) => {
